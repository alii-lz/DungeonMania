...
class MenuScene extends Scene {
    ...
    preload() {
        ...
        if (
            typeof window.Config.skinFile.main_menu.background_music == "string"
        ) {
            this.load.audio(
                "main-menu-music-",
            ...
            for (var audio in window.Config.skinFile.main_menu
                .background_music) {
    ...
    create() {
        ...
        this.newGame = new TextButton(
            ...
            async () => {
                ...
                const result = await Swal.fire({
                    title: window.Config.localisation.main_menu.buttons
                        .new_game,
                    heightAuto: false,
                    html: `
                        ...
                        let inputValidator = async (values) => {
                            if (!values || !values[0]) {
                                Swal.showValidationMessage(
                                    "You need to select a dungeon"
                                );
                                return false;
                            }
                            if (!values || !values[1]) {
                                Swal.showValidationMessage(
                                    "You need to select a config file"
                                );
                                return false;
                            }
                            return values;
        ...
        this.credits = new TextButton(
            this,
            10,
            this.renderer.height / 4 + this.newGame.height + 10 + 20 + 75,
